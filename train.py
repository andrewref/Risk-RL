...
def train(num_games: int, max_turns: int, log_int: int, seed: int, deal: bool) -> None:
    ...
    try:
        for ep in range(1, num_games + 1):
            ...
            switch_events = ppo_ai.switch_log                  # [(step,strat), ...]
            ...
            with COUNT_CSV.open("a") as f:
                f.write(f"{ep},{strat_counts['aggressive']},"
                            f"{strat_counts['defensive']},"
                            f"{strat_counts['balanced']},"
                            f"{strat_counts['random']}\n")
        ...
        if current_game:
            # call end() to flush PPO memory / save weights
            current_game.players["PPO"].ai.end()             # type: ignore
        return
