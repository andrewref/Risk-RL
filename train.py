...
- Stops every PyRisk game after a fixed turn cap.
...
AGENT_NAMES = ["PPO", "StupidAI", "DefensiveAI", "RandomAI"]
...
def load_history() -> list[dict]:
    if os.path.exists(TRACK_FILE):
        try:
            return json.load(open(TRACK_FILE, "r")).get("history", [])
        except Exception:
            return []
    return []
...
def play_with_limits(game: Game, max_turns: int) -> str:
    """Play until the turn cap, then return the winner (or fallback)."""
    has_kw = "max_turns" in inspect.signature(Game.play).parameters
    try:
        return game.play(max_turns=max_turns) if has_kw else game.play() or "Draw"
...
def train(num_games: int, max_turns: int, log_interval: int, seed: int, deal: bool):
    ...
    last_logged_rate: Optional[float] = history[-1]["win_rate"] if history else None
    ...
    try:
        for ep in range(1, num_games + 1):
            ...
            g.add_player("StupidAI", StupidAI)
            ...
            # accumulate PPO's per-strategy counts
            ...
            if ep % log_interval == 0 or ep == num_games:
                ...
                if last_logged_rate is not None:
                    delta = ppo_rate - last_logged_rate
                    LOG.info(
                        "PPO win rate: %.2f%% -> %.2f%% (change %+0.2f%%)",
                        last_logged_rate,
                        ppo_rate,
                        delta,
                    )
