...
class FourPlayerBalancedEnv(gym.Env):
    """Balanced PPO agent: cautious expansion with clear advantages."""
    ...
    def _legal_attacks(self):
        ...
        for atk in self.world.my_territories():
            for nbr in self.world.get_neighbors(atk):
                if self.world.is_enemy(nbr) and self.world.troops(atk) > self.world.troops(nbr) + 1:
                    acts.append((atk, nbr))
        return acts
