...
from strategies.reward_random import FourPlayerRandomEnv
from stable_baselines3.common.vec_env import DummyVecEnv

# Incremental PPO training with automatic resume
if __name__ == "__main__":
    CHECKPOINT_FOLDER = "checkpoints/random"
    FINAL_MODEL_PATH = os.path.join(CHECKPOINT_FOLDER, "ppo_random_final.zip")
    TIMESTEP_FILE = os.path.join(CHECKPOINT_FOLDER, "timesteps.txt")
    os.makedirs(CHECKPOINT_FOLDER, exist_ok=True)
    env = DummyVecEnv([lambda: FourPlayerRandomEnv()])
